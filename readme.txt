...
* Enhancement - Handle PayPal API address data returned as an array instead of an object #4516
* Enhancement - Remove/Rework Cart Simulation #4473
* Enhancement - Remove Pay with Crypto promotional pricing messaging #4540
* Enhancement - Add filters to allow limiting of ingested/valid products #4547
* Enhancement - Validators need extraction from CheckoutEndpoint #4370
* Fix - invoice_id missing for existing customers paying with saved PayPal payment tokens #4541
* Fix - Fractional WooCommerce quantities truncated to 0, causing invalid PayPal line items and checkout failures #4530
* Fix - fraudnet.js: Uncaught ReferenceError PAYPAL is not defined on classic checkout when hosted_fields_loaded fires before fb.js loads #4520
* Fix - Saved PayPal token in subscription change-payment flow moves active subscription to pending #4536
* Fix - PHP warning in PayPal order tracking when WooCommerce Shipment Tracking sync reads non-array tracking metadata #4505
* Fix - OXXO description is missing by default #4479
* Fix - CHECKOUT.ORDER.APPROVED webhook fails with uncaught PayPalOrderMissingException causing HTTP 500 loop, pending orders and Apple Pay never captured #4534
* Fix - Block Checkout "Sign up now" button label not replaced with "Proceed to PayPal" #4440
* Fix - ppcp-local-alternative-payment-methods-css-gateway.css loads on every frontend page #4465
* Fix - ACDC edit order page shows Processor Response 0000:Approved on edit order page #4478
...
* Fix - Store Sync toggle missing when feature flag is active #4535
* Fix - Inconsistent PayPal button behavior for subscriptions on cart #4542
* Fix - Scope reCAPTCHA assets and markup to active PayPal payment locations #4548
