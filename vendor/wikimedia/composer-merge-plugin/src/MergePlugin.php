...
class MergePlugin implements PluginInterface, EventSubscriberInterface
{
    ...
    public function activate(Composer $composer, IOInterface $io)
    {
        ...
        $this->state = new \Wikimedia\Composer\Merge\V2\PluginState($this->composer);
        $this->logger = new \Wikimedia\Composer\Merge\V2\Logger('merge-plugin', $io);
    }
    /**
     * {@inheritdoc}
     */
    ...
    public function onInit(BaseEvent $event)
    {
        ...
        $this->mergeFiles($this->state->getRequires(), \true);
    }
    /**
    ...
    public function onInstallUpdateOrDump(ScriptEvent $event)
    {
        ...
        if ($event->getName() === ScriptEvents::PRE_AUTOLOAD_DUMP) {
            $this->state->setDumpAutoloader(\true);
    ...
    protected function mergeFiles(array $patterns, $required = \false)
    {
        ...
        $files = array_map(static function ($files, $pattern) use ($required) {
            if ($required && !$files) {
                throw new \Wikimedia\Composer\Merge\V2\MissingFileException("merge-plugin: No files matched required '{$pattern}'");
            }
            return $files;
    ...
    protected function mergeFile(RootPackageInterface $root, $path)
    {
        ...
        $package = new \Wikimedia\Composer\Merge\V2\ExtraPackage($path, $this->composer, $this->logger);
        ...
        if (!empty($requirements)) {
            $this->updateAllowList = array_replace($this->updateAllowList, array_fill_keys($requirements, \true));
        }
        if ($this->state->isDevMode()) {
            $this->loaded[$path] = \true;
        } else {
            $this->loadedNoDev[$path] = \true;
        }
        if ($this->state->recurseIncludes()) {
            $this->mergeFiles($package->getIncludes(), \false);
            $this->mergeFiles($package->getRequires(), \true);
        }
    }
    /**
    ...
    public function onPostPackageInstall(PackageEvent $event)
    {
        ...
        if ($op instanceof InstallOperation) {
            ...
            if ($package === self::PACKAGE_NAME) {
                ...
                $this->state->setFirstInstall(\true);
    ...
    public function onPostInstallOrUpdate(ScriptEvent $event)
    {
        ...
        if ($this->state->isFirstInstall()) {
            ...
            if ($status !== 0 && $lockBackup && $lock && !$this->state->isComposer1()) {
                $this->logger->log("\n" . '<error>' . 'Update to apply merge settings failed, reverting ' . $lock . ' to its original content.' . '</error>');
