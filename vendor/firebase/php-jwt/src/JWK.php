...
class JWK
{
    ...
    public static function parseKey(array $jwk, string $defaultAlg = null): ?\Firebase\JWT\Key
    {
        ...
        switch ($jwk['kty']) {
            case 'RSA':
                ...
                return new \Firebase\JWT\Key($publicKey, $jwk['alg']);
            case 'EC':
                if (isset($jwk['d'])) {
            ...
            case 'OKP':
                ...
                $publicKey = \Firebase\JWT\JWT::convertBase64urlToBase64($jwk['x']);
                return new \Firebase\JWT\Key($publicKey, $jwk['alg']);
            default:
                break;
        }
    ...
    {
        $pem = self::encodeDER(self::ASN1_SEQUENCE, self::encodeDER(self::ASN1_SEQUENCE, self::encodeDER(self::ASN1_OBJECT_IDENTIFIER, self::encodeOID(self::OID)) . self::encodeDER(self::ASN1_OBJECT_IDENTIFIER, self::encodeOID(self::EC_CURVES[$crv]))) . self::encodeDER(self::ASN1_BIT_STRING, \chr(0x0) . \chr(0x4) . \Firebase\JWT\JWT::urlsafeB64Decode($x) . \Firebase\JWT\JWT::urlsafeB64Decode($y)));
    ...
    {
        $mod = \Firebase\JWT\JWT::urlsafeB64Decode($n);
        $exp = \Firebase\JWT\JWT::urlsafeB64Decode($e);
