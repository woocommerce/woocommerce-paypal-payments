...
class JWT
{
    ...
    public static function decode(string $jwt, $keyOrKeyArray, stdClass &$headers = null): stdClass
    {
        ...
        if (isset($payload->nbf) && floor($payload->nbf) > $timestamp + static::$leeway) {
            $ex = new \Firebase\JWT\BeforeValidException('Cannot handle token with nbf prior to ' . \date(DateTime::ISO8601, (int) $payload->nbf));
        ...
        if (!isset($payload->nbf) && isset($payload->iat) && floor($payload->iat) > $timestamp + static::$leeway) {
            $ex = new \Firebase\JWT\BeforeValidException('Cannot handle token with iat prior to ' . \date(DateTime::ISO8601, (int) $payload->iat));
            $ex->setPayload($payload);
            throw $ex;
        }
        // Check if this token has expired.
        if (isset($payload->exp) && $timestamp - static::$leeway >= $payload->exp) {
            $ex = new \Firebase\JWT\ExpiredException('Expired token');
            $ex->setPayload($payload);
            throw $ex;
        }
        return $payload;
    ...
    {
        if ($keyOrKeyArray instanceof \Firebase\JWT\Key) {
            return $keyOrKeyArray;
        }
        if (empty($kid) && $kid !== '0') {
            throw new UnexpectedValueException('"kid" empty, unable to lookup correct key');
        }
        if ($keyOrKeyArray instanceof \Firebase\JWT\CachedKeySet) {
