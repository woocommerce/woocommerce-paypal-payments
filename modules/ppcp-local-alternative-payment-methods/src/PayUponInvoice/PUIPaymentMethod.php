...
class PUIPaymentMethod extends AbstractPaymentMethodType {
	...
	private PayUponInvoiceGateway $gateway;

	public function __construct(
		AssetGetter $asset_getter,
		string $version,
		PayUponInvoiceGateway $gateway
	) {
		$this->asset_getter = $asset_getter;
		$this->version      = $version;
		$this->gateway      = $gateway;

		$this->name = PayUponInvoiceGateway::ID;
	}
	...
	public function get_payment_method_script_handles() {
		wp_register_script(
			'ppcp-pui-payment-method',
			$this->asset_getter->get_asset_url( 'pui-payment-method.js' ),
			array(),
			$this->version,
			true
		);
	...
	public function get_payment_method_data() {
		...
		return array(
			...
			'requiresPhone'        => $this->requires_phone(),
			'siteLanguage'         => $site_country_code,
			'placeOrderButtonText' => $button_text,
			'ratepayTerms'         => array(
				'de' => $this->ratepay_terms_de( $button_text ),
				'en' => $this->ratepay_terms_en( $button_text ),
			),
		);
	}
