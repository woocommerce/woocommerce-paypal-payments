...
class PayUponInvoiceGateway extends WC_Payment_Gateway
{
    ...
    private TransactionUrlProvider $transaction_url_provider;
    private LoggerInterface $logger;
    private CheckoutHelper $checkout_helper;
    private RefundProcessor $refund_processor;
    public function __construct(PayUponInvoiceOrderEndpoint $order_endpoint, PurchaseUnitFactory $purchase_unit_factory, \WooCommerce\PayPalCommerce\LocalAlternativePaymentMethods\PayUponInvoice\PaymentSourceFactory $payment_source_factory, Environment $environment, TransactionUrlProvider $transaction_url_provider, LoggerInterface $logger, CheckoutHelper $checkout_helper, bool $is_connected, RefundProcessor $refund_processor, AssetGetter $asset_getter)
    {
        $this->id = self::ID;
        $this->init_apm_defaults();
        $gateway_settings = get_option('woocommerce_' . self::ID . '_settings');
        $this->title = is_array($gateway_settings) && isset($gateway_settings['title']) ? $gateway_settings['title'] : $this->method_title;
        $this->description = is_array($gateway_settings) && isset($gateway_settings['description']) ? $gateway_settings['description'] : __('Once you place an order, pay within 30 days. Our payment partner Ratepay will send you payment instructions.', 'woocommerce-paypal-payments');
        ...
        $this->icon = apply_filters('woocommerce_paypal_payments_pay_upon_invoice_gateway_icon', $asset_getter->get_static_asset_url('images/ratepay.svg'));
        $this->supports = $is_connected ? array('refunds') : array('products');
    }
    /**
    ...
    public function process_payment($order_id)
    {
        ...
        $birth_date = is_string($birth_date) ? $birth_date : '';
        ...
        $payment_source_data = $this->payment_source_factory->from_wc_order($wc_order, $birth_date);
        try {
            $order = $this->order_endpoint->create(array($purchase_unit), $payment_source_data, $wc_order);
