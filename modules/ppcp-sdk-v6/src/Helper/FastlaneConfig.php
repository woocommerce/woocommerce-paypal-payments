...
class FastlaneConfig
{
     ...
     */
    private $is_eligible;
    public function __construct(CardPaymentsConfiguration $card_payments_configuration, SubscriptionHelper $subscription_helper, callable $is_eligible)
    {
    ...
    public function should_render(string $context): bool
    {
        ...
        if (!did_action('wp_loaded')) {
            _doing_it_wrong(__METHOD__, esc_html__('Fastlane availability cannot be determined before the wp_loaded action has run.', 'woocommerce-paypal-payments'), '4.1.3');
            return \false;
        }
        if (!in_array($context, self::SUPPORTED_CONTEXTS, \true)) {
            return \false;
        }
