...
class GooglePayConfig
{
    ...
    public function __construct(SettingsProvider $settings_provider, SubscriptionHelper $subscription_helper, callable $is_available)
    {
    ...
    public function should_render(string $context): bool
    {
        ...
        if (!did_action('wp_loaded')) {
            _doing_it_wrong(__METHOD__, esc_html__('Google Pay availability cannot be determined before the wp_loaded action has run.', 'woocommerce-paypal-payments'), '4.1.3');
            return \false;
        }
    ...
    private function enabled_for_context(string $context): bool
    {
        ...
        return $styling->enabled && in_array(GooglePayGateway::ID, $styling->methods, \true);
    }
    /**
