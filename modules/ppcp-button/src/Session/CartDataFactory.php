...
class CartDataFactory
{
    ...
    public function from_current_cart(?WC_Cart $cart = null): \WooCommerce\PayPalCommerce\Button\Session\CartData
    {
        ...
        $cart_data = new \WooCommerce\PayPalCommerce\Button\Session\CartData($cart->get_cart_for_session(), $cart->get_applied_coupons(), $cart->needs_shipping(), get_current_user_id(), $cart->get_cart_hash(), $this->fees($cart));
        if (WC()->session) {
    ...
    protected function fees(WC_Cart $cart): array
    {
        ...
        return array_map(
            ...
            static function ($fee): array {
                return array('id' => (string) ($fee->id ?? ''), 'name' => (string) ($fee->name ?? ''), 'taxable' => (bool) ($fee->taxable ?? \false), 'tax_class' => (string) ($fee->tax_class ?? ''), 'amount' => (float) ($fee->amount ?? 0), 'total' => (float) ($fee->total ?? 0), 'tax' => (float) ($fee->tax ?? 0), 'tax_data' => (array) ($fee->tax_data ?? array()));
            },
            $fees
        );
    }
