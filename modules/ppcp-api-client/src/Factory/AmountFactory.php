...
class AmountFactory
{
    ...
    public function from_wc_cart(\WC_Cart $cart): Amount
    {
        ...
        list($taxes_cents, $discount_cents) = $this->reconcile_with_total((int) round($wc_total * 100), (int) round($item_total_val * 100), (int) round($shipping_val * 100), (int) round($taxes_val * 100), (int) round($discount_val * 100));
    ...
    public function from_store_api_cart(CartTotals $cart_totals): Amount
    {
        ...
        list($tax_minor, $discount_minor) = $this->reconcile_with_total($total_minor, $items_minor, $shipping_minor, $tax_minor, $discount_minor);
    ...
    public function from_wc_order(\WC_Order $order): Amount
    {
        ...
        if ((in_array($order->get_payment_method(), array(CreditCardGateway::ID, CardButtonGateway::ID), \true) || PayPalGateway::ID === $order->get_payment_method() && 'card' === $order->get_meta(PayPalGateway::ORDER_PAYMENT_SOURCE_META_KEY)) && $this->is_free_trial_order($order)) {
            ...
            list($taxes_cents, $adjusted_discount_cents) = $this->reconcile_with_total((int) round($wc_total * 100), (int) round($item_total_val * 100), (int) round($shipping_val * 100), (int) round($taxes_val * 100), $discount_cents);
     ...
     */
    private function reconcile_with_total(int $total_minor, int $items_minor, int $shipping_minor, int $taxes_minor, int $discount_minor): array
    {
        ...
        return array($adjusted_taxes, $items_minor + $shipping_minor + $adjusted_taxes - $total_minor);
    }
    /**
