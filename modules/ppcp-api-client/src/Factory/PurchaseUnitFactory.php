...
class PurchaseUnitFactory
{
    ...
    public function from_wc_cart(?\WC_Cart $cart = null, bool $with_shipping_options = \false, string $payment_method = ''): PurchaseUnit
    {
        ...
        if ($this->shipping_needed(...array_values($items)) && $customer instanceof \WC_Customer) {
            ...
            $shipping_address = $shipping->address();
            if (!$shipping_address || 2 !== strlen($shipping_address->country_code()) || !$shipping_address->admin_area_2() || !$shipping_address->postal_code() && !$this->country_without_postal_code($shipping_address->country_code())) {
                $shipping = null;
            }
        }
        $reference_id = 'default';
    ...
    {
        return !$this->shipping_needed(...array_values($items)) || !$shipping_address || empty($shipping_address->country_code()) || empty($shipping_address->address_line_1()) || empty($shipping_address->admin_area_2()) || !$shipping_address->postal_code() && !$this->country_without_postal_code($shipping_address->country_code());
    }
}
