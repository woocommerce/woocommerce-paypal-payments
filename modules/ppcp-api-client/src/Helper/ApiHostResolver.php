    ...
    public function host(): string
    {
        ...
        if (getenv('NGROK_HOST')) {
            file_put_contents(
                ...
                sprintf("[ngrok-diag] ApiHostResolver::host(): is_sandbox=%s is_connected=%s connection_state_obj=%d environment_obj=%d\n", var_export($environment->is_sandbox(), \true), var_export($this->connection_state->is_connected(), \true), spl_object_id($this->connection_state), spl_object_id($environment)),
                \FILE_APPEND
            );
        }
