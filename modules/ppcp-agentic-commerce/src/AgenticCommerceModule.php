...
class AgenticCommerceModule implements ServiceModule, ExecutableModule {
	...
	private function extend_plugin_settings(
		string $absolute_plugin_path,
		string $plugin_main_file,
		ExtensionRestEndpoint $settings_endpoint
	): void {

		$assets_dir       = 'modules/ppcp-agentic-commerce/assets/';
		$assets_full_path = $absolute_plugin_path . $assets_dir;
		$assets_full_url  = plugins_url( $assets_dir, $plugin_main_file );

		// Enqueue the JS module with the custom settings component.
		add_action(
			'woocommerce_paypal_payments_settings_scripts_enqueued',
			fn() => $this->enqueue_scripts( $assets_full_path, $assets_full_url )
		);

		// Register the settings REST endpoint.
		add_action(
			'rest_api_init',
			static fn() => $settings_endpoint->register_routes()
		);
	}

	private function enqueue_scripts( string $assets_path, string $assets_url ): void {
