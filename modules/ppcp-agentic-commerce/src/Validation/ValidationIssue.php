...
abstract class ValidationIssue
{
     ...
     */
    public function __construct(string $message, string $user_message = '', string $field = '', string $item_id = '', array $context = array(), array $resolution_options = array())
    {
        if (static::ISSUE_CODE === '' || static::ISSUE_TYPE === '') {
            throw new RuntimeException(sprintf('%s must define ISSUE_CODE and ISSUE_TYPE constants.', static::class));
        }
    ...
    public function add_resolution(string $action, string $label, string $url = '', array $metadata = array()): self
    {
        if (count($this->resolution_options) < self::MAX_RESOLUTION_OPTIONS) {
            $resolution = array('action' => $action, 'label' => $label);
            if ($url) {
    ...
    {
        $data = array('code' => $this->code(), 'type' => $this->type(), 'message' => substr($this->message, 0, self::MAX_MESSAGE_LENGTH));
