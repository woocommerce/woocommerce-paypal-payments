...
class TestAgentPage {
	...
	public function render(): void {
		...
		?>
		<div class="wrap">
			<h2><?php esc_html_e( 'PayPal Agentic Commerce - Test Agent', 'woocommerce-paypal-payments' ); ?></h2>
			<p class="description">
				<?php esc_html_e( 'Test the agentic commerce API by simulating AI agent interactions. This tool uses a browser-based AI model to interact with the real WooCommerce and PayPal APIs.', 'woocommerce-paypal-payments' ); ?>
			</p>
	...
	private function render_production_warning(): void {
		...
		<div class="wrap">
			...
			<div class="notice notice-warning">
				<p><?php esc_html_e( 'Test Agent is only available when connected to PayPal sandbox. Please connect to sandbox mode to use this testing tool.', 'woocommerce-paypal-payments' ); ?></p>
			</div>
		</div>
		<?php
	}
	...
	private function enqueue_assets(): void {
		...
		wp_enqueue_script(
			'ppcp-dummy-agent',
			$this->module_url . 'assets/dummy-agent.js',
			$asset['dependencies'],
			$asset['version'],
			true
		);
		...
		wp_enqueue_style(
			'ppcp-dummy-agent',
			$this->module_url . 'assets/dummy-agent.css',
			array(),
			$asset['version']
		);
		...
		wp_localize_script(
			...
			'ppcpDummyAgent',
			array(
				'agenticUrl'    => rest_url( 'wc/v3/agentic' ),
				'productsUrl'   => rest_url( 'wc/v3/products' ),
				'nonce'         => wp_create_nonce( 'wp_rest' ),
				'currency'      => get_woocommerce_currency(),
				'isSandbox'     => true,
				'testScenarios' => apply_filters( 'woocommerce_paypal_payments_agentic_commerce_test_scenarios', array() ),
			)
		);
	}
}
