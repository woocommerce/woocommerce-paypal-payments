...
class IngestionManager
{
    ...
    private \WooCommerce\PayPalCommerce\AgenticCommerce\Ingestion\IngestionBatchProvider $batch_provider;
    private AgenticWebhookConfiguration $webhook_urls;
    private MerchantMetadataProvider $metadata_provider;
    private LoggerInterface $logger;
    public function __construct(IngestionConfiguration $configuration, \WooCommerce\PayPalCommerce\AgenticCommerce\Ingestion\IngestionBatchProvider $batch_provider, AgenticWebhookConfiguration $webhook_urls, MerchantMetadataProvider $metadata_provider, LoggerInterface $logger)
    {
     ...
     */
    private function create_new_sync_job(array $product_ids): \WooCommerce\PayPalCommerce\AgenticCommerce\Ingestion\SyncJob
    {
        $metadata = $this->metadata_provider->get_metadata();
        return new \WooCommerce\PayPalCommerce\AgenticCommerce\Ingestion\SyncJob($this->webhook_urls->get_product_ingestion_url(), $metadata->store_url, $product_ids, $this->logger);
    }
}
