...
class ProductsPayload
{
    ...
    private function transform_products(array $product_ids): array
    {
        ...
        foreach ($product_ids as $product_id) {
            ...
            $api_product = array('id' => (string) $product->get_id(), 'title' => $product->get_name(), 'link' => $product->get_permalink(), 'image_link' => wp_get_attachment_image_url((int) $product->get_image_id(), 'full') ?: '', 'description' => $product->get_description() ?: $product->get_short_description(), 'price' => $this->format_price($product->get_price()), 'availability' => $this->map_stock_status($product->get_stock_status()), 'merchantStoreUrl' => $this->merchant_store_url);
            // Add optional fields.
            if ($product->get_sku()) {
    ...
    private function get_product_variants(WC_Product $variable_product): array
    {
        ...
        foreach ($variation_ids as $variation_id) {
            ...
            $variant = array('id' => (string) $variation->get_id(), 'item_group_id' => (string) $variable_product->get_id(), 'title' => $variation->get_name(), 'link' => $variation->get_permalink(), 'image_link' => (wp_get_attachment_image_url((int) $variation->get_image_id(), 'full') ?: wp_get_attachment_image_url((int) $variable_product->get_image_id(), 'full')) ?: '', 'description' => $variation->get_description() ?: $variable_product->get_description(), 'price' => $this->format_price($variation->get_price()), 'availability' => $this->map_stock_status($variation->get_stock_status()), 'merchantStoreUrl' => $this->merchant_store_url);
