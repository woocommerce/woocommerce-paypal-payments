...
class CouponResolutionBuilder
{
     ...
     * @var array
     */
    private const RESOLUTIONS = array('try_different' => array('action' => 'RETRY_REQUEST', 'label' => 'Try a different coupon code', 'metadata' => array('priority' => 'high')), 'remove' => array('action' => 'REMOVE_COUPON', 'label' => 'Continue without coupon', 'metadata' => array('priority' => 'medium')), 'modify_cart' => array('action' => 'MODIFY_CART', 'label' => 'Add eligible items to use this coupon', 'metadata' => array('priority' => 'high')), 'view_available' => array('action' => 'VIEW_AVAILABLE_COUPONS', 'label' => 'View available offers', 'metadata' => array('priority' => 'low')), 'suggest_alternative' => array('action' => 'SUGGEST_ALTERNATIVE_COUPON', 'label' => 'Try a different coupon', 'metadata' => array('priority' => 'medium')), 'add_items_to_minimum' => array('action' => 'ADD_ITEMS_TO_REACH_MINIMUM', 'label' => 'Add %s more to qualify', 'metadata' => array('priority' => 'high')), 'continue_without' => array('action' => 'CONTINUE_WITHOUT_COUPON', 'label' => 'Continue without coupon', 'metadata' => array('priority' => 'low')));
    /**
     ...
     */
    public function build_resolution_options(string $issue_type, array $keys, string $code, array $context, PayPalCart $cart, ?WC_Coupon $wc_coupon): array
    {
    ...
    private function build_stacking_resolutions(string $code, array $context, PayPalCart $cart): array
    {
        ...
        return array(array('action' => 'KEEP_CURRENT_COUPON', 'label' => sprintf('Keep %s (saves %s)', $code, $formatted_current), 'metadata' => array('priority' => 'high', 'savings' => $formatted_current)), array('action' => 'RETRY_REQUEST', 'label' => sprintf('Switch to %s (saves %s)', $attempted_coupon, $formatted_attempted), 'metadata' => array('priority' => 'low', 'savings' => $formatted_attempted)));
    }
}
