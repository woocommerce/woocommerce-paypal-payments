...
class CouponContextBuilder
{
    ...
    public function __construct(ProductManager $product_manager, \WooCommerce\PayPalCommerce\AgenticCommerce\CartValidation\CouponValidator\DiscountCalculator $discount_calculator)
    {
    ...
    private function build_minimum_spend(string $code, PayPalCart $cart, ?WC_Coupon $wc_coupon, array $extra): array
    {
        ...
        return array('minimum_required' => number_format($minimum, 2, '.', ''), 'current_subtotal' => number_format($subtotal, 2, '.', ''), 'shortage_amount' => number_format($shortage, 2, '.', ''), 'currency_code' => $currency);
    }
    /**
    ...
    private function build_maximum_spend(string $code, PayPalCart $cart, ?WC_Coupon $wc_coupon, array $extra): array
    {
        ...
        $currency = CartHelper::currency($cart, get_woocommerce_currency());
        return array('maximum_allowed' => number_format($maximum, 2, '.', ''), 'current_subtotal' => number_format($subtotal, 2, '.', ''), 'currency_code' => $currency);
    }
    /**
