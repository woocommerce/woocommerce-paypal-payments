...
class PriceValidator implements \WooCommerce\PayPalCommerce\AgenticCommerce\CartValidation\ValidatorInterface
{
    ...
    private function create_price_mismatch_issue($product, $cart_price, float $store_price, string $field, PayPalCart $cart): PriceMismatch
    {
        ...
        return new PriceMismatch(sprintf("Price mismatch for '%s': cart price is %s but store price is %s", $product->get_name(), $cart_price->value(), $store_price), sprintf('The price of %s has %s from %s to %s.', $product->get_name(), $is_increase ? 'increased' : 'decreased', CartHelper::format_price((string) $cart_price->value(), $cart), CartHelper::format_price((string) $store_price, $cart)), $field, '', $context, $resolution_options);
    }
    private function build_mismatch_context($cart_price, float $store_price, float $price_difference, bool $is_increase): array
    {
        $context = array('specific_issue' => 'PRICE_MISMATCH', 'original_price' => CartHelper::format_decimal($cart_price->value()), 'current_price' => CartHelper::format_decimal($store_price), 'currency_code' => $cart_price->currency_code());
        if ($is_increase) {
    ...
    {
        return array(array('action' => 'ACCEPT_NEW_PRICE', 'label' => sprintf('Continue with %s', CartHelper::format_price((string) $store_price, $cart)), 'metadata' => array('cost_impact' => sprintf('%s%s', $is_increase ? '+' : '-', CartHelper::format_price((string) abs($price_difference), $cart)), 'priority' => 'high')), array('action' => 'REMOVE_ITEM', 'label' => 'Remove from cart', 'metadata' => array('cost_impact' => sprintf('-%s', CartHelper::format_price((string) $cart_price->value(), $cart)), 'priority' => 'medium')));
    }
}
