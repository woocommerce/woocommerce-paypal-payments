...
class CurrencyValidator implements \WooCommerce\PayPalCommerce\AgenticCommerce\CartValidation\ValidatorInterface
{
    public function validate(PayPalCart $cart)
    {
        ...
        $store_issue = $this->validate_store_currency($cart_currencies[0]['currency'], $cart_currencies[0]['index']);
    ...
    private function validate_consistent_currency(array $currencies): ?CurrencyMismatch
    {
        ...
        return new CurrencyMismatch(sprintf('Mixed currencies detected: item %d has currency %s, expected %s', $mismatch['index'], $mismatch['currency'], $reference['currency']), 'All items in the cart must use the same currency.', "items[{$mismatch['index']}].price.currency_code");
    }
    /**
    ...
    private function validate_store_currency(string $cart_currency, int $item_index): ?CurrencyMismatch
    {
        ...
        if ($cart_currency !== $store_currency) {
            return new CurrencyMismatch(sprintf('Cart currency %s does not match store currency %s', $cart_currency, $store_currency), sprintf('This store only accepts payments in %s.', $store_currency), "items[{$item_index}].price.currency_code");
        }
        return null;
    }
}
