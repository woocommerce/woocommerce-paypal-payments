 ...
 */
class CartItem extends \WooCommerce\PayPalCommerce\AgenticCommerce\Schema\AgenticSchema
{
    ...
    private ?\WooCommerce\PayPalCommerce\AgenticCommerce\Schema\Money $price = null;
    private ?array $selected_attributes = null;
    private ?\WooCommerce\PayPalCommerce\AgenticCommerce\Schema\GiftOptions $gift_options = null;
    protected function parse_fields(array $input, callable $add_issue): void
    {
        ...
        if (isset($input['price']) && is_array($input['price'])) {
            $price = \WooCommerce\PayPalCommerce\AgenticCommerce\Schema\Money::from_array($input['price'], $add_issue);
            if ($price->value() <= 0.0) {
        ...
        if (isset($input['gift_options']) && is_array($input['gift_options'])) {
            $this->gift_options = \WooCommerce\PayPalCommerce\AgenticCommerce\Schema\GiftOptions::from_array($input['gift_options'], $add_issue);
        }
    ...
    public function price(): ?\WooCommerce\PayPalCommerce\AgenticCommerce\Schema\Money
    {
        return $this->price;
    }
    ...
    public function gift_options(): ?\WooCommerce\PayPalCommerce\AgenticCommerce\Schema\GiftOptions
    {
