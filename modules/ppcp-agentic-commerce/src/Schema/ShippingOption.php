 ...
 */
class ShippingOption extends \WooCommerce\PayPalCommerce\AgenticCommerce\Schema\AgenticSchema
{
    private string $id = '';
    private string $name = '';
    ...
    protected function parse_fields(array $input, callable $add_issue): void
    {
        ...
        if (!isset($input['price']) || !is_array($input['price'])) {
            ...
            $money = \WooCommerce\PayPalCommerce\AgenticCommerce\Schema\Money::from_array($input['price'], $add_issue);
            $issues = $money->issues();
            if (empty($issues)) {
                $this->price = $money;
            } else {
