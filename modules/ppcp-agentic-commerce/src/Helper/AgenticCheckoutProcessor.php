...
class AgenticCheckoutProcessor
{
    ...
    private function build_payer_data(PayPalCart $cart): array
    {
        ...
        if ($cart->billing_address()) {
            $payer_data['address'] = \WooCommerce\PayPalCommerce\AgenticCommerce\Helper\CartHelper::billing_address_array($cart);
        }
        return $payer_data;
    }
    /**
    ...
    private function build_shipping_data(PayPalCart $cart): array
    {
        ...
        return array('name' => array('full_name' => \WooCommerce\PayPalCommerce\AgenticCommerce\Helper\CartHelper::full_customer_name($cart)), 'address' => \WooCommerce\PayPalCommerce\AgenticCommerce\Helper\CartHelper::shipping_address_array($cart));
    }
    /**
