...
class AgenticCartBuilder
{
    ...
    private \WooCommerce\PayPalCommerce\AgenticCommerce\Helper\ProductManager $product_manager;
    private CartDataFactory $cart_data_factory;
    private PurchaseUnitFactory $purchase_unit_factory;
    private LoggerInterface $logger;
    public function __construct(WooCommerce $wc, \WooCommerce\PayPalCommerce\AgenticCommerce\Helper\ProductManager $product_manager, CartDataFactory $cart_data_factory, PurchaseUnitFactory $purchase_unit_factory, LoggerInterface $logger)
    {
        $this->wc = $wc;
    ...
    private function set_addresses(WC_Customer $wc_customer, PayPalCart $paypal_cart): void
    {
        if ($paypal_cart->shipping_address()) {
            $shipping = \WooCommerce\PayPalCommerce\AgenticCommerce\Helper\CartHelper::shipping_address_array($paypal_cart);
        ...
        if ($paypal_cart->billing_address()) {
            $billing = \WooCommerce\PayPalCommerce\AgenticCommerce\Helper\CartHelper::billing_address_array($paypal_cart);
