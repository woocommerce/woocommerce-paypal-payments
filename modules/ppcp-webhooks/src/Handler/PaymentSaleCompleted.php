...
class PaymentSaleCompleted implements \WooCommerce\PayPalCommerce\Webhooks\Handler\RequestHandler
{
    ...
    public function handle_request(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if ($subscriptions) {
            ...
            $this->logger->info(sprintf('PAYMENT.SALE.COMPLETED (transaction %1$s) matched WC subscription(s) #%2$s via billing agreement %3$s.', $transaction_id, $subscription_ids, $billing_agreement_id));
            try {
            ...
            $this->logger->warning(sprintf('PAYMENT.SALE.COMPLETED (transaction %1$s) arrived for billing agreement %2$s but no WC subscription with a matching "ppcp_subscription" meta was found; no renewal order was created.', $transaction_id, $billing_agreement_id));
        }
