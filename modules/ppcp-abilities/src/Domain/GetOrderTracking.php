...
class GetOrderTracking extends \WooCommerce\PayPalCommerce\Abilities\Domain\AbstractPpcpAbility implements AbilityDefinition
{
    ...
    {
        return array('label' => __('Get PayPal order tracking', 'woocommerce-paypal-payments'), 'description' => __('Returns the shipment tracking entries (carrier, tracking number, status) registered with PayPal for a WooCommerce order. Issues two synchronous PayPal API calls.', 'woocommerce-paypal-payments'), 'category' => AbilityNames::CATEGORY_SLUG, 'input_schema' => array('type' => 'object', 'default' => (object) array(), 'properties' => array('wc_order_id' => array('type' => 'integer', 'minimum' => 1, 'description' => __('The WooCommerce order ID whose tracking entries should be returned.', 'woocommerce-paypal-payments'))), 'required' => array('wc_order_id'), 'additionalProperties' => \false), 'execute_callback' => AbilityHandlers::callback(self::get_name()), 'permission_callback' => AbilityHandlers::permission_callback(), 'meta' => array('annotations' => array('readonly' => \true, 'destructive' => \false, 'idempotent' => \true), 'show_in_rest' => \true, 'mcp' => array('public' => \true)));
    }
}
