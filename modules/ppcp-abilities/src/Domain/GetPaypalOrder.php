...
class GetPaypalOrder extends \WooCommerce\PayPalCommerce\Abilities\Domain\AbstractPpcpAbility implements AbilityDefinition
{
    ...
    {
        return array('label' => __('Get PayPal order', 'woocommerce-paypal-payments'), 'description' => __('Returns the PayPal order (id, intent, status, purchase units with embedded captures / authorizations / refunds, create/update timestamps, links) for a given PayPal order ID or WooCommerce order ID. Payer PII (email, name, address, phone) and per-purchase-unit shipping addresses are stripped by default; pass include_payer_pii: true to opt in when the calling context legitimately needs them.', 'woocommerce-paypal-payments'), 'category' => AbilityNames::CATEGORY_SLUG, 'input_schema' => array('type' => 'object', 'default' => (object) array(), 'properties' => array('paypal_order_id' => array('type' => 'string', 'description' => __('PayPal v2 order ID (alphanumeric uppercase, up to 64 chars). Either this or wc_order_id is required.', 'woocommerce-paypal-payments')), 'wc_order_id' => array('type' => 'integer', 'minimum' => 1, 'description' => __('WooCommerce order ID; the PayPal order ID is resolved from order meta. Either this or paypal_order_id is required.', 'woocommerce-paypal-payments')), 'include_payer_pii' => array('type' => 'boolean', 'default' => \false, 'description' => __('When true, returns the payer block (email, name, address, phone) and per-purchase-unit shipping addresses. Defaults to false; only opt in when the calling context legitimately needs payer identity. Sites can deny payer PII outright via the woocommerce_paypal_payments_abilities_allow_payer_pii filter, in which case this flag has no effect.', 'woocommerce-paypal-payments'))), 'additionalProperties' => \false), 'execute_callback' => AbilityHandlers::callback(self::get_name()), 'permission_callback' => AbilityHandlers::permission_callback(), 'meta' => array('annotations' => array('readonly' => \true, 'destructive' => \false, 'idempotent' => \true), 'show_in_rest' => \true, 'mcp' => array('public' => \true)));
    }
}
