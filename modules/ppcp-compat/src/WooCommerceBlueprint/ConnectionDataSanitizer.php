...
class ConnectionDataSanitizer
{
     ...
     * @var array<string, mixed>
     */
    public const ONBOARDING_DEFAULTS = array('completed' => \false, 'step' => 0, 'gateways_synced' => \false, 'gateways_refreshed' => \false);
    /**
    ...
    public function sanitize(array $options): array
    {
        if (isset($options[self::OPTION_COMMON]) && is_array($options[self::OPTION_COMMON])) {
            $options[self::OPTION_COMMON] = $this->reset_present_keys($options[self::OPTION_COMMON], self::CONNECTION_DEFAULTS);
        }
        if (isset($options[self::OPTION_ONBOARDING]) && is_array($options[self::OPTION_ONBOARDING])) {
            $options[self::OPTION_ONBOARDING] = $this->reset_present_keys($options[self::OPTION_ONBOARDING], self::ONBOARDING_DEFAULTS);
        }
        return $options;
    }
    /**
