...
class SettingsImporter {
	...
	public function maybe_hook() {
		...
		if ( PPECHelper::is_gateway_available() && false === get_option( SettingsModel::OPTION_KEY ) ) {
	...
	private function import_basic_settings(): void {
		foreach ( $this->ppec_settings as $option_key => $option_value ) {
			switch ( $option_key ) {
				...
				case 'landing_page':
					$landing_page = strtolower( $option_value ) === 'login' ? 'login' :
						( strtolower( $option_value ) === 'billing' ? 'guest_checkout' : 'any' );
					$this->settings_model->set_landing_page( $landing_page );
					break;

				case 'paymentaction':
					if ( 'authorization' === $option_value ) {
						$this->settings_model->set_authorize_only( true );
					}
					break;
	...
	private function import_styling_settings(): void {
		...
		foreach ( self::CONTEXT_MAP as $old_context => $new_context ) {
			...
			$location_styles[ $new_context ] = new LocationStylingDTO(
				$new_context,
				$this->get_button_enabled_state( $old_context ),
				$this->enabled_methods( $old_context ),
				$this->get_button_property( 'shape', $old_context, $use_cart_settings ) ?: 'rect',
				$this->get_button_property( 'label', $old_context, $use_cart_settings ) ?: 'paypal',
				$this->get_button_property( 'color', $old_context, $use_cart_settings ) ?: 'gold',
				$this->get_button_property( 'layout', $old_context, $use_cart_settings ) ?: 'vertical',
				false
			);
		}
	...
	private function enabled_methods( string $old_context ): array {
		...
		if ( isset( $this->ppec_settings['hide_funding_methods'] ) && is_array( $this->ppec_settings['hide_funding_methods'] ) ) {
			if ( ! in_array( 'venmo', array_map( 'strtolower', $this->ppec_settings['hide_funding_methods'] ), true ) ) {
				$methods[] = 'venmo';
			}
		}
