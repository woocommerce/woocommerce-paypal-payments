...
class SettingsImporter
{
    ...
    public function maybe_hook()
    {
        ...
        if (\WooCommerce\PayPalCommerce\Compat\PPEC\PPECHelper::is_gateway_available() && \false === get_option($this->ppcp_settings::KEY)) {
    ...
    {
        static $context_translations = array('' => 'cart', 'mark' => '', 'single_product' => 'product', 'mini_cart' => 'mini-cart');
        static $credit_messaging_translations = array('credit_message_layout' => 'layout', 'credit_message_logo' => 'logo', 'credit_message_logo_position' => 'position', 'credit_message_text_color' => 'color', 'credit_message_flex_color' => 'flex_color', 'credit_message_flex_ratio' => 'flex_ratio');
        $result = array();
        foreach ($this->ppec_settings as $option_key => $option_value) {
            ...
            switch ($option_key) {
                ...
                case 'hide_funding_methods':
                    $key = 'disable_funding';
                    $value = array_values(array_intersect(array_map('strtolower', is_array($option_value) ? $option_value : array()), array('card', 'sepa', 'bancontact', 'blik', 'eps', 'giropay', 'ideal', 'mercadopago', 'mybank', 'p24', 'sofort', 'venmo', 'trustly')));
                    break;
        ...
        if (isset($this->ppec_settings['credit_enabled']) && 'no' === $this->ppec_settings['credit_enabled']) {
            $result['disable_funding'] = array_merge(isset($result['disable_funding']) ? $result['disable_funding'] : array(), array('credit'));
        }
