...
class ProductsPayload
{
    ...
    private function transform_products(array $product_ids): array
    {
        ...
        foreach ($product_ids as $product_id) {
            ...
            $api_product = array('id' => (string) $product->get_id(), 'title' => $this->get_product_title($product), 'link' => $this->get_product_link($product), 'image_link' => $this->get_product_image($product), 'description' => $this->get_product_description($product, $product->get_short_description()), 'price' => $this->format_price($product->get_price()), 'availability' => $this->get_product_availability($product), 'merchantStoreUrl' => $this->merchant_store_url);
            // Add optional fields.
            if ($product->get_sku()) {
    ...
    private function get_product_variants(WC_Product $variable_product): array
    {
        ...
        foreach ($variation_ids as $variation_id) {
            ...
            $variant = array('id' => (string) $variation->get_id(), 'item_group_id' => (string) $variable_product->get_id(), 'title' => $this->get_product_title($variation), 'link' => $this->get_product_link($variation), 'image_link' => $this->get_product_image($variation, wp_get_attachment_image_url((int) $variable_product->get_image_id(), 'full') ?: ''), 'description' => $this->get_product_description($variation, $variable_product->get_description()), 'price' => $this->format_price($variation->get_price()), 'availability' => $this->get_product_availability($variation), 'merchantStoreUrl' => $this->merchant_store_url);
    ...
    {
        $image_url = wp_get_attachment_image_url((int) $product->get_image_id(), 'full') ?: $fallback;
        /**
