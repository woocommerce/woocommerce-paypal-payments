...
class CartItem extends AgenticSchema {
	...
	protected function parse_fields( array $input, StoreValidation $validation ): void {
		...
		if ( isset( $input['item_id'] ) && is_string( $input['item_id'] ) ) {
			...
			if ( strlen( $id ) > 127 ) {
				$validation->add_invalid_data(
					'item_id',
					'Item id too long',
					'The item ID can be at most 127 characters long'
				);
			} else {
				$this->id = $id;
			}
		...
		if ( isset( $input['variant_id'] ) && is_string( $input['variant_id'] ) ) {
			...
			if ( strlen( $variant_id ) > 127 ) {
				$validation->add_invalid_data(
					'variant_id',
					'Variant id too long',
					'The variant ID can be at most 127 characters long'
				);
			} else {
		...
		if ( isset( $input['parent_id'] ) && is_string( $input['parent_id'] ) ) {
			...
			if ( strlen( $parent_id ) > 127 ) {
				$validation->add_invalid_data(
					'parent_id',
					'Parent id too long',
					'The parent ID can be at most 127 characters long'
				);
			} else {
		...
		if ( isset( $input['name'] ) && is_string( $input['name'] ) ) {
			...
			if ( strlen( $name ) > 127 ) {
				$validation->add_invalid_data(
					'name',
					'Item name too long',
					'The item name can be at most 127 characters long'
				);
			} else {
				$this->name = $name;
			}
		...
		if ( isset( $input['description'] ) && is_string( $input['description'] ) ) {
			...
			if ( strlen( $description ) > 255 ) {
				$validation->add_invalid_data(
					...
					'The item description can be at most 127 characters long'
				);
			} else {
		...
		if ( isset( $input['price'] ) && is_array( $input['price'] ) ) {
			$price = Money::from_array( $input['price'], $validation );
		...
		if ( isset( $input['gift_options'] ) && is_array( $input['gift_options'] ) ) {
			$this->gift_options = GiftOptions::from_array( $input['gift_options'], $validation );
		}
		...
		if ( isset( $input['selected_attributes'] ) && is_array( $input['selected_attributes'] ) ) {
			...
			if ( count( $attributes ) > 10 ) {
				$validation->add_invalid_data(
					'selected_attributes',
					'Too many attributes',
					'The item can have at most 10 attributes'
				);
			} else {
