...
class PayPalCart extends \WooCommerce\PayPalCommerce\StoreSync\Schema\AgenticSchema
{
    ...
    protected function parse_fields(array $input, callable $add_issue): void
    {
        ...
        if (!empty($input['items']) && is_array($input['items'])) {
            ...
            if (count($items) > 100) {
                ...
                foreach ($items as $item) {
                    ...
                    $this->items[] = \WooCommerce\PayPalCommerce\StoreSync\Schema\CartItem::from_array($item, $add_issue);
                }
            }
        } else {
        ...
        if (!empty($input['customer']) && is_array($input['customer'])) {
            $this->customer = \WooCommerce\PayPalCommerce\StoreSync\Schema\Customer::from_array($input['customer'], $add_issue);
        }
        if (!empty($input['shipping_address']) && is_array($input['shipping_address'])) {
            $this->shipping_address = \WooCommerce\PayPalCommerce\StoreSync\Schema\Address::from_array($input['shipping_address'], $add_issue);
        }
        if (!empty($input['billing_address']) && is_array($input['billing_address'])) {
            $this->billing_address = \WooCommerce\PayPalCommerce\StoreSync\Schema\Address::from_array($input['billing_address'], $add_issue);
        }
        if (!empty($input['geo_coordinates']) && is_array($input['geo_coordinates'])) {
            $this->geo_coordinates = \WooCommerce\PayPalCommerce\StoreSync\Schema\GeoCoordinates::from_array($input['geo_coordinates'], $add_issue);
        }
