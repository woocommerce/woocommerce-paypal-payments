	...
	protected function parse_fields( array $input, StoreValidation $validation ): void {
			...
			if ( is_float( $latitude ) ) {
				if ( $latitude < - 90.0 || $latitude > 90.0 ) {
					$validation->add_invalid_data(
						'latitude',
						'Invalid latitude',
						'Latitude must be a decimal value between -90.0 and 90.0'
					);
				} else {
			...
			if ( is_float( $longitude ) ) {
				if ( $longitude < - 180.0 || $longitude > 180.0 ) {
					$validation->add_invalid_data(
						'longitude',
						'Invalid longitude',
						'Longitude must be a decimal value between -180.0 and 180.0'
					);
				} else {
		...
		if ( isset( $input['subdivision'] ) && is_string( $input['subdivision'] ) ) {
			...
			if ( strlen( $subdivision ) > 10 ) {
				$validation->add_invalid_data(
					'subdivision',
					'Subdivision too long',
					'The subdivision code must be in ISO 3166-2 format (no country code).'
				);
			} elseif ( ! preg_match( '/^[A-Z0-9-]+$/', $subdivision ) ) {
				$validation->add_invalid_data(
					'subdivision',
					'Subdivision invalid',
					'The subdivision code must be in ISO 3166-2 format.'
				);
			} else {
		...
		if ( isset( $input['country_code'] ) && is_string( $input['country_code'] ) ) {
			...
			if ( ! preg_match( '/^[A-Z]{2}$/', $country_code ) ) {
				$validation->add_invalid_data(
					...
					'The country code must be a 2-letter value.'
				);
			} else {
