...
class CurrencyValidator implements ValidatorInterface {
	...
	private function validate_currency( StorePayPalCart $store_cart ): ?ValidationIssue {
		$mismatch = array_filter(
			...
			static fn( StoreCartItem $item ) => ! $item->is_currency_correct()
		);
		...
		return ValidationIssue::create_currency_mismatch(
			sprintf(
				...
				$field->assumed_currency(),
				$store_cart->currency()
			)
		)
			->user_message(
				sprintf( 'This store only accepts payments in %s.', $store_cart->currency() )
			)
			->for_field( $field->field_path( 'price.currency_code' ) )
			->add_resolution(
				ResolutionOption::create_use_different_currency()
					->label( sprintf( 'Change to %s', $store_cart->currency() ) )
					->set_meta( 'expected_currency', $store_cart->currency() )
			);
	}
}
