...
class CouponResolutionBuilder
{
    ...
    private function build_resolution_by_key(string $key, array $context, PayPalCart $cart): ?ResolutionOption
    {
        switch ($key) {
            ...
            case 'modify_cart':
                return ResolutionOption::modify_cart('Add eligible items to use this coupon', array('priority' => Priority::HIGH));
            case 'view_available':
                return ResolutionOption::redirect_to_merchant('View available offers', '', array('priority' => Priority::LOW));
            case 'suggest_alternative':
                return ResolutionOption::apply_different_coupon('Try a different coupon', Priority::MEDIUM);
            case 'add_items_to_minimum':
                $formatted_amount = isset($context['shortage_amount']) ? CartHelper::format_price($context['shortage_amount'], $cart) : '';
                return ResolutionOption::modify_cart(sprintf('Add %s more to qualify', $formatted_amount), array('priority' => Priority::HIGH, 'amount_needed' => $formatted_amount));
    ...
    private function build_stacking_resolutions(string $code, array $context, PayPalCart $cart): array
    {
        ...
        return array(ResolutionOption::keep_current_coupon(sprintf('Keep %s (saves %s)', $code, $formatted_current), Priority::HIGH, array('savings' => $formatted_current)), ResolutionOption::apply_different_coupon(sprintf('Switch to %s (saves %s)', $attempted_coupon, $formatted_attempted), Priority::LOW, array('savings' => $formatted_attempted)));
    }
}
