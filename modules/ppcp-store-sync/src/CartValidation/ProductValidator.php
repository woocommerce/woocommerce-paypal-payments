...
class ProductValidator implements \WooCommerce\PayPalCommerce\StoreSync\CartValidation\ValidatorInterface
{
    ...
    private function validate_product(int $key, CartItem $item): ?ValidationIssue
    {
        ...
        if (!$support_downloads && $product->is_downloadable()) {
            return new InvalidProduct("Downloadable product '{$identifier}' is not supported", "'{$item->name()}' cannot be purchased at this time", $field);
        }
        if (!$product->is_type($valid_types)) {
            return new InvalidProduct("Product '{$identifier}' is not supported (unsupported product type)", "'{$item->name()}' cannot be purchased at this time", $field);
        }
        if (!in_array($product->get_status(), $valid_status, \true)) {
            return new InvalidProduct("Product '{$identifier}' is not supported (product has an unsupported status)", "'{$item->name()}' cannot be purchased at this time", $field);
        }
        return null;
    }
}
