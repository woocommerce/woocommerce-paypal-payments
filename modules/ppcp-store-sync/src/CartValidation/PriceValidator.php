...
class PriceValidator implements ValidatorInterface {
		...
		return ValidationIssue::create_price_mismatch(
			...
			->user_message(
				sprintf(
					...
					Money::create( (string) $cart_price->value(), $currency )->to_price(),
					Money::create( (string) $store_price, $currency )->to_price()
				)
			)
			->for_field( $field )
			->add_context( $this->build_mismatch_context( $cart_price, $store_price, $price_difference, $is_increase ) )
			->add_resolution( $this->build_resolution_options( $cart_price, $store_price, $price_difference, $is_increase, $currency ) );
	}
	...
	private function build_mismatch_context( Money $cart_price, float $store_price, float $price_difference, bool $is_increase ): PricingErrorContext {
		$context = PricingErrorContext::create_price_mismatch()
			->original_price( Money::create( $cart_price->value() ?? 0. )->to_decimal() )
			->current_price( Money::create( $store_price )->to_decimal() )
	...
	private function build_resolution_options( Money $cart_price, float $store_price, float $price_difference, bool $is_increase, string $currency ): array {
		$formatted_difference = sprintf(
			'%s%s',
			$is_increase ? '+' : '-',
			Money::create( (string) abs( $price_difference ), $currency )->to_price()
		);
		...
		return array(
			ResolutionOption::create_accept_new_price()
				->label( sprintf( 'Continue with %s', Money::create( (string) $store_price, $currency )->to_price() ) )
