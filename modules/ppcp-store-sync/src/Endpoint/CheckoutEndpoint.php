...
class CheckoutEndpoint extends \WooCommerce\PayPalCommerce\StoreSync\Endpoint\AgenticRestEndpoint
{
    ...
    public function complete_checkout(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (is_wp_error($order)) {
            $store_cart->validation()->add_payment_error($order->get_error_message())->add_context(PaymentErrorContext::create_payment_declined()->decline_reason((string) $order->get_error_code()));
    ...
    {
        return $this->checkout_processor->process($store_cart, $store_cart->paypal_cart()->payment_method(), $paypal_order_id);
    }
}
