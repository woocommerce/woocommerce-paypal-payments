	...
	private function update_connection_details( MerchantConnectionDTO $connection ): void {
		...
		if ( $this->common_settings->is_merchant_connected() ) {
			...
			file_put_contents( // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents -- temporary diagnostic.
				...
				sprintf(
					"[ngrok-diag] connect() done: is_sandbox=%s is_connected=%s connection_state_obj=%d environment_obj=%d\n",
					var_export( $connection->is_sandbox, true ),
					var_export( $this->connection_state->is_connected(), true ),
					spl_object_id( $this->connection_state ),
					spl_object_id( $this->connection_state->get_environment() )
				),
				FILE_APPEND
			);
