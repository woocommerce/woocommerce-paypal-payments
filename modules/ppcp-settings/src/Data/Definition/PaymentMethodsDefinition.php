...
class PaymentMethodsDefinition
{
     ...
     */
    public function __construct(PaymentSettings $settings, GeneralSettings $general_settings, string $axo_checkout_config_notice = '', string $axo_incompatible_plugins_notice = '', bool $sdk_v6_active = \false)
    {
    ...
    public function get_definitions(): array
    {
        ...
        foreach ($all_methods as $method) {
            $method_id = $method['id'];
            $result[$method_id] = $this->build_method_definition($method_id, $method['title'], $method['description'], $method['icon'], $method['fields'] ?? array(), $method['warningMessages'] ?? array(), $method['warningSeverity'] ?? 'warning');
        }
        return $result;
    }
    /**
     ...
     */
    private function build_method_definition(string $gateway_id, string $title, string $description, string $icon, $fields = array(), array $warning_messages = array(), string $warning_severity = 'warning'): array
    {
        ...
        $config = array('id' => $gateway_id, 'enabled' => $enabled, 'title' => str_replace('&amp;', '&', $gateway_title), 'description' => $gateway_description, 'icon' => $icon, 'itemTitle' => $title, 'itemDescription' => $description, 'warningMessages' => $warning_messages, 'warningSeverity' => $warning_severity);
        if (is_array($fields)) {
    ...
    public function group_card_methods(): array
    {
        ...
        if (!$this->sdk_v6_active) {
            $card_fields['cardholderName'] = array('type' => 'toggle', 'default' => $this->settings->get_cardholder_name(), 'label' => __('Display cardholder name', 'woocommerce-paypal-payments'));
        }
        $card_fields['showCardLogos'] = array('type' => 'toggle', 'default' => $this->settings->get_show_card_logos(), 'label' => __('Show logos of supported cards', 'woocommerce-paypal-payments'));
        if (!$this->general_settings->own_brand_only()) {
            $group[] = array('id' => CreditCardGateway::ID, 'title' => __('Advanced Credit and Debit Card Payments', 'woocommerce-paypal-payments'), 'description' => __("Present custom credit and debit card fields to your payers so they can pay with credit and debit cards using your site's branding.", 'woocommerce-paypal-payments'), 'icon' => 'payment-method-advanced-cards', 'fields' => $card_fields);
    ...
    private function get_warning_messages(): array
    {
        ...
        $warnings[PayUponInvoiceGateway::ID] = array('pui_required_fields' => array(
            /* translators: %s: comma-separated list of missing field names. */
            'message' => __('Pay upon Invoice requires %s to be configured. Click the settings icon to configure.', 'woocommerce-paypal-payments'),
