...
class ResumedOrderShippingRestorer
{
    ...
    public function snapshot(int $order_id): void
    {
        try {
            ...
            $this->logger->error(sprintf('Could not snapshot the shipping line items of resumed order %1$d: %2$s', $order_id, $exception->getMessage()));
        }
    }
    /**
    ...
    public function restore(int $order_id): void
    {
        ...
        try {
            ...
            $this->logger->error(sprintf('Could not restore the shipping line item of resumed order %1$d, which has a shipping total of %2$s and no shipping line item backing it: %3$s', $order_id, '' !== $shipping_total ? $shipping_total : 'unknown', $exception->getMessage()));
        }
    }
    /**
    ...
    private function item_from(array $item_data): WC_Order_Item_Shipping
    {
        ...
        if (is_array($meta)) {
            foreach ($meta as $meta_entry) {
                $item->add_meta_data((string) $meta_entry['key'], $meta_entry['value'], \true);
            }
        }
        return $item;
    }
