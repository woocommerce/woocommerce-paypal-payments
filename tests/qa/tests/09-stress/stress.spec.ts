...
const testProduct = products.simpleWithStock;
...
test.beforeAll( async ( { utils, pcpApi, wooCommerceApi } ) => {
	...
	const { id: productId } =
		await wooCommerceApi.getProductBySlug( testProduct.slug );
...
for( let i = 1; i <= 500; i++ ) {
	...
	test(
		...
		async ( {
			...
			let orderProduct = await wooCommerceApi.getProductBySlug( productSlug );
			...
			await expect( orderStatus, `Order #${ orderId } status` ).toBe( 'processing' );

			// Verify product stock quantity
			orderProduct = await wooCommerceApi.getProductBySlug( productSlug );
			await expect( orderProduct.stock_quantity ).toBe( expectedProductStock );
			...
			await expect( paymentViaNote.length, 'Payment via order notes quantity' ).toBe( 1 );
			await expect( transactionNote.length, 'Transaction order notes quantity' ).toBe( 1 );
		}
	);
}
