...
test( 'PCP-6237 | Settings - Overview - Tab loads and all navigation tabs are visible @Critical', async ( {
	pcpOverview,
} ) => {
...
test( 'PCP-6236 | Settings - Overview - Todos card structure, dismiss, and restore @Critical', async ( {
	pcpOverview,
} ) => {
	await pcpOverview.visit();
	await pcpOverview.waitForOverview();
	...
	await test.step( 'Assert item count, titles, and dismiss buttons', async () => {
		...
		const dismissCount = await pcpOverview.todoDismissButtons().count();
		expect(
			dismissCount,
	...
	await test.step( 'Dismiss first todo item and assert it is removed from the list', async () => {
		...
		await expect(
			pcpOverview.todoItems().filter( { hasText: firstTitle } ).first(),
			'Assert the dismissed todo item is removed from the list'
		).not.toBeAttached( { timeout: 5_000 } );
	} );

	await test.step( 'Restore dismissed items and assert success notice', async () => {
...
test( 'PCP-6235 | Settings - Overview - Clicking a todo item navigates to its associated tab @Critical', async ( {
	pcpOverview,
} ) => {
	await pcpOverview.visit();
	await pcpOverview.waitForOverview();
	...
	await fastlaneTodo.locator( '.ppcp-r-todo-item__description' ).click();
