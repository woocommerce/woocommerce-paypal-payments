...
class PayPalPaymentMethodTest extends TestCase
{
    ...
    /**
     * GIVEN a free-trial subscription cart with a vaultable subscription (vault token saving
     *      allowed), for either a logged-in customer or a guest, with or without an existing
     *      saved PayPal or Venmo account
     * WHEN get_payment_method_data() is called
     * THEN the standard "Place order" row stays enabled, since a first-time buyer without a
     *      saved account is now handled by the gateway's server-side vault-approval redirect
     *      rather than requiring an existing token up front
     */
    public function testPlaceOrderEnabledForFreeTrialCart(): void
    {
