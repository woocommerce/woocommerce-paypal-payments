...
class PayLaterWCBlocksRendererTest extends TestCase
{
     ...
     *       resolves true), and the renderer was configured (via its constructor, not
     *       block attributes) with a flex layout and its flex/text style values
     * WHEN the cart Pay Later placement is rendered
    ...
    public function testCoercesToTextLayoutWhenV6OwnsTheCurrentPageEvenWithFlexLayoutConfig(): void
    {
        ...
        $html = $renderer->render(
            array( 'ppcpId' => 'ppcp-cart-paylater-messages', 'blockId' => 'woocommerce-paypal-payments/cart-paylater-messages' ),
            'cart',
            $container
        );
     ...
     *       registered in the container), and the renderer was configured with a flex
     *       layout
     * WHEN the cart Pay Later placement is rendered
