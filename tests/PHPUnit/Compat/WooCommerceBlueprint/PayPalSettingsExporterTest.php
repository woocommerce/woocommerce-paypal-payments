...
class PayPalSettingsExporterTest extends TestCase {
	...
	/**
	 * The escape hatch for automated environments where no merchant can opt in
	 * through the UI.
	 *
	 * @test
	 */
	public function test_filter_forces_credentials_into_the_default_export(): void {
		when( 'apply_filters' )->alias(
			function ( string $hook, $value ) {
				return 'woocommerce_paypal_payments_blueprint_export_connection_details' === $hook
					? true
					: $value;
			}
		);
	...
	/**
	 * The filter changes the payload only. Selection identity must stay put,
	 * otherwise the alias the UI posts would stop resolving.
	 *
	 * @test
	 */
	public function test_filter_does_not_change_selection_identity(): void {
		when( 'apply_filters' )->justReturn( true );

		$exporter = new PayPalSettingsExporter( new ConnectionDataSanitizer(), false );

		self::assertSame( PayPalSettingsExporter::ALIAS, $exporter->get_alias() );
		self::assertSame( SetPayPalSettings::get_step_name(), $exporter->get_step_name() );
	}
